...
class Detector:
    ...
    def onImage(self, imagePath=False, image_mat=None, savePath="/home/ytpc2017d/catkin_ws/src/object_detector/images/save.jpg"):
        if (image_mat!=None):
            image=image_mat
        if imagePath:
    ...
    def onVideo(self,videoPath,savePath=False,csvPath=False):
        ...
        while success:
            ...
            if csvPath:
                try:
                    np_pred_boxes=predictions["instances"].pred_boxes.tensor.cpu().numpy()[0]
                    #np_pred_boxes=pred_boxes.to(torch.device('cpu')).clone().numpy()[0]
                    #.detach().clone().numpy()[0]
                    #print(dir(np_pred_boxes))
                    ...
                    try:
                        ...
                        np_pred_boxes=np.full((1,4),np.nan)
                    np_pred_boxes_time=np.insert(np_pred_boxes,0,i)
                    history.append(np_pred_boxes_time)
    ...
    def onROS(self,topicName='/ouster/signal_image'):
