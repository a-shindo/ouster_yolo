...
videos=sorted(glob("/home/ytpc2017d/catkin_ws/src/object_detector/scripts/temp/results/*"))

"""
for videoPath in videos:
    video_basename=os.path.basename(videoPath)
    if video_basename[-4:]==".mp4":
        detector.onVideo(videoPath=videoPath,savePath=f"/home/ytpc2017d/catkin_ws/src/object_detector/scripts/temp/results2/{video_basename}v2")
...
class Detector:
    ...
    def onVideo(self,videoPath,savePath=False,csvPath=False):
        ...
        while success:
            ...
            if csvPath:
                try:
                    np_pred_boxes=predictions["instances"].pred_boxes.tensor.cpu().numpy()[0]#np_pred_boxes=pred_boxes.to(torch.device('cpu')).clone().numpy()[0]#.detach().clone().numpy()[0]
                    print(np_pred_boxes)
                    np_pred_boxes_time=np.insert(np_pred_boxes,0,i)
                    history.append(np_pred_boxes_time)
                except IndexError:
                    try:
                        np_pred_boxes=np.full_like(np_pred_boxes,np.nan)
                    except UnboundLocalError:
                        np_pred_boxes=np.full((1,51),np.nan)
                    np_pred_boxes_time=np.insert(np_pred_boxes,0,i)
                    history.append(np_pred_boxes_time)
